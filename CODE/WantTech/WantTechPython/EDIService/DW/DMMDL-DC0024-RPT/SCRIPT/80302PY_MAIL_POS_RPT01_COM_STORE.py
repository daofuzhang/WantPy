...
def getPOSRPT01StorePCSList(strEDIConnection, strEDIDB, strDataDate):
    strSQL = "SELECT KA_SYSTEM_CODE, KA_SYSTEM_NM, KA_SYSTEM_ACT_NM " + \
             ...
             "WHERE CONVERT(POS_WEEK_NUM, INT)>=(CONVERT(LEFT({DATA_DATE}, 4), INT) - 1)*100+49 " + \
             "ORDER BY SALES_COM_ID_SA, KA_SYSTEM_CODE, SALES_COM_ID_WH, KA_STORE_CODE, POS_WEEK_NUM DESC; "
...
def execWriteWorkbook(strJobPathRES, listWeekNum, listPOSRPT01, listPOSRPT01Com, listPOSRPT01ComPCS, listPOSRPT01ComSKU, listPOSRPT01StorePCS, listPOSRPT01StoreSKU):
    if len(listWeekNum) == 0 or len(listPOSRPT01) == 0 or len(listPOSRPT01StorePCS) != len(listPOSRPT01StoreSKU) or len(listPOSRPT01ComPCS) != len(listPOSRPT01ComSKU):
        return False
    else:
        ...
        while intPOSRPT01StoreIndex < len(listPOSRPT01StorePCS):
            if strSALES_COM_ID_SA != listPOSRPT01StorePCS[intPOSRPT01StoreIndex]["SALES_COM_ID_SA"]:
                strSALES_COM_ID_SA = listPOSRPT01StorePCS[intPOSRPT01StoreIndex]["SALES_COM_ID_SA"]
                strSALES_COM_ABR_SA = listPOSRPT01StorePCS[intPOSRPT01StoreIndex]["SALES_COM_ABR_SA"]
                ...
                objExcelWorksheet.merge_range("A1:I1", "现代渠道发展营业部业绩－分公司系统门店周别POS推移", objExcelTitleFormat)
            ...
            for dictWeekNum in listWeekNum:
                ...
                if intIndexTemp < len(listPOSRPT01StorePCS) and \
                   listPOSRPT01StorePCS[intIndexTemp]["SALES_COM_ID_SA"] == strSALES_COM_ID_SA and \
                   ...
                   listPOSRPT01StorePCS[intIndexTemp]["POS_WEEK_NUM"] == listWeekNum[intTemp]["WEEK_NUM"]:
                    ...
                    if (intIndexTemp + 1) < len(listPOSRPT01StorePCS) and (intTemp + 1) < len(listWeekNum) and \
                       listPOSRPT01StorePCS[intIndexTemp + 1]["SALES_COM_ID_SA"] == strSALES_COM_ID_SA and \
            ...
            for dictWeekNum in listWeekNum:
                ...
                if intIndexTemp < len(listPOSRPT01StoreSKU) and \
                   listPOSRPT01StoreSKU[intIndexTemp]["SALES_COM_ID_SA"] == strSALES_COM_ID_SA and \
                   ...
                   listPOSRPT01StoreSKU[intIndexTemp]["POS_WEEK_NUM"] == listWeekNum[intTemp]["WEEK_NUM"]:
                    ...
                    if (intIndexTemp + 1) < len(listPOSRPT01StoreSKU) and (intTemp + 1) < len(listWeekNum) and \
                       listPOSRPT01StoreSKU[intIndexTemp + 1]["SALES_COM_ID_SA"] == strSALES_COM_ID_SA and \
...
def execMailPOSRPT01(strJobPathRES, strDataDate):
    ...
    if listPOSRPT01[0]["POS_DATE"] == listPOSRPT01[0]["WEEK_LAST_DATE"]:
        ...
        if execWriteWorkbook(strJobPathRES, listWeekNum, listPOSRPT01, listPOSRPT01Com, listPOSRPT01ComPCS, listPOSRPT01ComSKU, listPOSRPT01StorePCS, listPOSRPT01StoreSKU) == True:
            ...
            for dictPOSRPT01Com in listPOSRPT01Com:
                strMessageSubject = dictPOSRPT01Com["SALES_COM_ABR_SA"] + "-现代渠道发展营业部业绩-分公司系统门店周别POS推移-" + listPOSRPT01[0]["POS_DATE"]
                strContent = "尊敬的长官：早上好！\n敬请查阅：截止" + listPOSRPT01[0]["POS_DATE"][4:6] + "月" + listPOSRPT01[0]["POS_DATE"][-2:] + "日【" + dictPOSRPT01Com["SALES_COM_ABR_SA"] + "-现代渠道发展营业部业绩-分公司系统门店周别POS推移报表】\n\n"
                strAttachFilePath = strJobPathRES.replace(".dat", "-" + dictPOSRPT01Com["SALES_COM_ID_SA"] + ".dat")
                strAttachFileName = dictPOSRPT01Com["SALES_COM_ABR_SA"] + "-现代渠道发展营业部业绩-分公司系统门店周别POS推移-" + listPOSRPT01[0]["POS_DATE"] + ".xlsx"
