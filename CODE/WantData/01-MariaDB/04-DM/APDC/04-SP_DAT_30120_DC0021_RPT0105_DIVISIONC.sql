DROP PROCEDURE IF EXISTS APDC.SP_DAT_30120_DC0021_RPT0105_DIVISIONC;

CREATE PROCEDURE APDC.SP_DAT_30120_DC0021_RPT0105_DIVISIONC (
...
BEGIN
    ...
    INSERT INTO APDC.DC0021_RPT0105_DIVISIONC A
    ...
    WHERE DATA_DATE=VAR_DATA_DATE AND RPT_YMD=VAR_DATA_DATE
      AND DIVISION_ID IN (@DIVISION_ID_MILK, @DIVISION_ID_LEIS)
    ;

    --UPDATE2
    --WORK_RATIO 標準進度
    UPDATE APDC.DC0021_RPT0105_DIVISIONC A
    ...
    JOIN(SELECT SUM(CASE WHEN RPT_YMD=VAR_DATA_DATE THEN FCST_AMT ELSE 0 END) AS FCST_AMT
         ...
         WHERE DATA_DATE=VAR_DATA_DATE AND RPT_YMD=VAR_DATA_DATE
         AND DIVISION_ID IN (@DIVISION_ID_MILK, @DIVISION_ID_LEIS)) B
    ON A.DATA_DATE=B.DATA_DATE AND A.RPT_YMD=B.RPT_YMD
    ...
    JOIN(SELECT DIVISIONC_ID
              ...
              , (CASE WHEN ROUND((SALE_AMT/SALES_AMT_PAST_MONTH)-1,3)<0 THEN 'A'
                      WHEN FYBG_SALES_AMT_PAST_MONTH<=0 THEN 'B' ELSE NULL END) AS FYBG_PAST_GROWTH_RATE_F
              ...
              , (CASE WHEN RPT_YMD=@RPT_YMD_END THEN 'Z'
                      WHEN ROUND((SALES_AMT_REACH_MONTH/SALES_AMT_PAST_PROG)-1,3)<0 THEN 'A'
         ...
         FROM APDC.DC0021_RPT0105_DIVISIONC
