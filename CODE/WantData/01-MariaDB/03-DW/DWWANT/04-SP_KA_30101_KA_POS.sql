DROP PROCEDURE IF EXISTS DWWANT.SP_KA_30101_KA_POS;

CREATE PROCEDURE DWWANT.SP_KA_30101_KA_POS (
    VAR_EDI_NO CHAR(12), VAR_DATA_DATE CHAR(8), OUT VAR_SP_RESULT CHAR(1)
)
BEGIN

    INSERT INTO DWWANT.TRAS_KA_POS
    SELECT VAR_EDI_NO AS EDI_NO, VAR_DATA_DATE AS DATA_DATE
         ...
         , (CASE WHEN K.MIN_PROD_UNIT='' THEN NULL ELSE K.MIN_PROD_UNIT END) AS SYSTEM_PROD_UNIT
         ...
         , (CASE WHEN K.MEANS_DATE='' THEN NULL ELSE K.MEANS_DATE END) AS POS_CRAWL_DATE
         , (CASE WHEN K.MEANS_TIME='' THEN NULL ELSE K.MEANS_TIME END) AS POS_CRAWL_TIME
         , (CASE WHEN K.IS_MEANS='Y' THEN 'O' ELSE 'F' END) AS POS_SOURCE
         , (CASE WHEN K.MATCH_STATUS IS NULL THEN NULL ELSE CONVERT(K.MATCH_STATUS, CHAR) END) AS POS_STATUS
         , (CASE WHEN K.SALE_DIVISION='' THEN NULL ELSE K.SALE_DIVISION END) AS POS_TYPE
         , (CASE WHEN K.END_MONTH_FLAG=1 THEN 'M'
                 WHEN K.END_MONTH_FLAG=0 THEN 'D'
                 ELSE NULL END) AS POS_SETTLE_TYPE
         , (CASE WHEN K.SALE_DATE='' THEN NULL ELSE K.SALE_DATE END) AS POS_DATE
         , (CASE WHEN K.DAY_SALE_NUM=-99999 THEN NULL ELSE K.DAY_SALE_NUM END) AS POS_QTY_PCS
         , (CASE WHEN K.SALE_BOX_NUM=-99999 THEN NULL ELSE K.SALE_BOX_NUM END) AS POS_QTY_PKG
         , (CASE WHEN K.DAY_SALE_PRICE=-99999 THEN NULL ELSE K.DAY_SALE_PRICE END) AS POS_AMT
    FROM RAWWANT.KA_WANTWANT_SYS_SALE_DATA K
    ;
